...
public class Toys {
    ...
    private static String[] getTopQuoted(int numToys, int topToys, String[] toys, int numQuotes, String[] quotes) {
        ...
        PriorityQueue<Toy> priorityQueue = new PriorityQueue<>((t1, t2) -> t2.count - t1.count == 0? t2.name.compareToIgnoreCase(t1.name):t2.count - t1.count);
        for (int i = 0; i < numQuotes; i++) {
            ...
            for (int j = 0; j < numToys; j++) {
                final String toyName = toys[j].toLowerCase();
                if (stringSet.contains(toyName)) {
                   // map.merge(toys[j], 1, Integer::sum);\
    ...
    public static void main(String[] args) {
        ...
        try (Scanner scanner = new Scanner(new File("/Users/Oleksandr/Documents/nexus/csv/us-one-worders2/part-00000-f16bbcad-5883-4e5a-8a66-8a98e58cefff-c000.csv"));) {
            scanner.nextLine();
            while (scanner.hasNextLine()) {
        ...
        for (String toy : getTopQuoted(10000, 10, keywordArray, quotes.size(), quotesArray)) {
