...
public class Task4d {
    ...
    public static int solution(int[] dimensions, int[] your_position, int[] guard_position, int distance) {
        ...
        Map<Key, Long> angles = new HashMap<>();
        ...
        while (!queue.isEmpty()) {
            ...
            final long d1 = g - your_position[0];
            final long d2 = h - your_position[1];
            final long c1 = x - your_position[0];
            final long c2 = y - your_position[1];
            long d = d1 * d1 + d2 * d2;
            if (d > distance * distance) {
                continue;
            }
            final Key a = key(c1, c2);
            final Key key = key(d1, d2);
    ...
    private static Key key(long c1, long c2) {
        if (c1 == 0 && c2 == 0) {
            return new Key(0, 0);
        } else if (c1 == 0) {
            return c2 > 0 ? new Key(0, 1) : new Key(0, -1);
        } else if (c2 == 0) {
            return c1 > 0 ? new Key(1, 0) : new Key(-1, 0);
        } else {
            long g = gcd(c1, c2);
            return new Key(c1 / g, c2 / g);
        }
    }
