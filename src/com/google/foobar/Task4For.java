...
public class Task4For {
    ...
    private static class Beam{
        ...
        public Beam(long distSq, boolean toGuard) {
            this.distSq = distSq;
    ...
    public static int solution(int[] dimensions, int[] your_position, int[] guard_position, int distance) {
        ...
        Map<Double, Beam> angles = new HashMap<>();
        int count = 0;
        ...
        for (int i = -n; i < n + 1; i++) {
            ...
            for (int j = -m; j < m + 1; j++) {
                ...
                int x = mirror(your_position[0], i, width);
                int y = mirror(your_position[1], j, height);

                int g = mirror(guard_position[0], i, width);
                int h = mirror(guard_position[1], j, height);
                ...
                if(c <= distance * distance && c > 0){
                    ...
                    if(angles.get(a)!=null){
                        if(angles.get(a).distSq>=c){
                            angles.put(a,new Beam(c, false));
                        }
                    }else {
                        angles.put(a,new Beam(c, false));
                    }
                ...
                if(d <= distance * distance && d > 0){
                    if(angles.get(key)!=null){
                        if(angles.get(key).distSq>=d){
                          //  System.out.println("("+g +","+h +")");
                            angles.put(key,new Beam(d, true));
                        }
                    }else {
                       // System.out.println("("+g +","+h +")");
                        angles.put(key,new Beam(d, true));
        ...
        for (Map.Entry<Double, Beam> entry :angles.entrySet()) {
            if(entry.getValue().toGuard) count++;
        }
        return count;
